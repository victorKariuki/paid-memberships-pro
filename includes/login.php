...
function pmpro_logged_in_welcome($show_menu = true, $show_logout_link = true)
{
	if (is_user_logged_in()) {
		 ...
		 * The menu can be customized per level using the Nav Menus Add On for Paid Subscriptions Pro.
		 *
		 */
		if (!empty($show_menu)) {
		 ...
		 * User will be redirected to the Subscription Account page if no other redirect intercepts the process.
		 *
		 */
		if (!empty($show_logout_link)) { ?>
			<div class="<?php echo pmpro_get_element_class('pmpro_member_log_out'); ?>"><a
					href="<?php echo esc_url(wp_logout_url()); ?>"><?php esc_html_e('Log Out', 'paid-memberships-pro'); ?></a></div>
			<?php
		}
	}
}
