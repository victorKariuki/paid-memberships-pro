...
function pmpro_search_filter($query)
{
	...
	if( !$query->is_admin &&
        ...
        if( $hidden_cat_ids ) {
			...
			// Check if the current category IDs are arrays.
			$current_cat_ids = $query->get('category__in');
			if (!is_array($current_cat_ids)) {
				$current_cat_ids = array();
			}

			// Show only these categories to members.
			$merged_cat_ids = array_merge($current_cat_ids, array_diff($all_cat_ids, $hidden_cat_ids));
			$query->set('category__in', $merged_cat_ids);
	...
	if( $hasaccess ) {
		...
		if( pmpro_getOption( "showexcerpts" ) ) {
			...
			if( $post->post_excerpt ) {
				...
				$excerpt_length = apply_filters('excerpt_length', 105);
